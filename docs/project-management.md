...
- 当前版本：浏览器插件 `1.0.1` 已冻结；macOS 原型 `0.7.3`，`0.7.2` 为本轮修改前回滚点，`0.3.0` 为 ⭐ 重要稳定版
- 当前目标：验收恢复后的张开手掌左右挥动翻页和既有捏合导航
...
| 04 | 进行中 | macOS 系统级手势控制原型 | `0.7.3` 已恢复灵活的张开手掌翻页，等待真实手势验收 |
...
| 开发 | 功能实现、修 Bug、验证 | macOS `0.7.3` 捏合导航与张开手掌翻页 |
...
- 在 Chrome、Safari、Edge、夸克验证严格 OK 左向右返回、右向左前进；确认非浏览器不接收按键。
- 实测严格 OK 垂直滚动不会因横向漂移误触导航，近中线起手、斜向和未跨中线均不触发。
- 实测张开手掌向右挥动下翻、向左挥动上翻；确认无需跨中线，食指和 V 手势不翻页。
