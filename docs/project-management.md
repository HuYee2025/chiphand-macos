...
- 当前版本：浏览器插件 `1.0.1` 已冻结；macOS 原型 `0.8.1`，`0.8.0` 为本轮修改前回滚点，`0.3.0` 为 ⭐ 重要稳定版
- 当前目标：验收浏览器食指悬停、张开拇指点击和当前页打开稳定性
...
| 04 | 进行中 | macOS 系统级手势控制原型 | `0.8.1` 食指指针与拇指点击已实现，等待 YouTube 实机验收 |
...
| 开发 | 功能实现、修 Bug、验证 | macOS `0.8.1` 浏览器食指指针与拇指点击 |
...
- 验证食指停稳后张开拇指只点击一次且始终在当前页打开；收回拇指后才能再次点击，普通张掌仍能左右翻页。
