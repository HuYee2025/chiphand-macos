...
- 当前版本：浏览器插件 `1.0.1` 已冻结；macOS 原型 `0.8.4`，`0.8.3` 为本轮修改前回滚点，`0.3.0` 为 ⭐ 重要稳定版
...
| 04 | 进行中 | macOS 系统级手势控制原型 | `0.8.4` 跨中线蓝色闪光反馈已实现，等待实机视觉验收 |
...
| 开发 | 功能实现、修 Bug、验证 | macOS `0.8.4` 跨中线闪光反馈 |
