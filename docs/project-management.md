...
- 当前版本：浏览器插件 `1.0.1` 已冻结；macOS 原型 `0.1.0`
...
| 开发 | 功能实现、修 Bug、验证 | macOS `0.1.0` 已实现并完成命令行构建与签名 |
...
- 安装完整 Xcode；当前 Command Line Tools 已可构建，但缺少标准测试/调试环境。
- 打开 `macos-app/build/GestureControl.app`，授予摄像头与辅助功能权限。
- 在 Chrome、Safari、Preview、Notion 验证左挥上翻、右挥下翻和捏住上下拖动滚动。
