...
- 当前阶段：第 05 阶段——薯片手开源发布（已完成）
- 当前版本：浏览器插件 `1.0.1` 已冻结；macOS `1.1.1 build 24` 本地待实机验收，公开 Release 仍为 `macos-v1.0.1`，`macos-v0.3.0` 为 ⭐ 重要稳定版
- 当前目标：维护公开仓库、Release 和 `chiphand.huyee.art`，继续接收用户实机反馈和后续细节优化
...
| 04 | 已完成 | macOS 系统级手势控制原型 | `docs/dev-log/archive/04-macos-system-gesture-prototype.md` |
| 05 | 已完成 | 薯片手开源发布 | `HuYee2025/chiphand-macos` 已公开，`macos-v1.0.1` Release 已发布 |
...
| 开发 | 功能实现、修 Bug、验证 | `1.1.1` 双手检测、控制手优先、标签稳定窗口和单一菜单栏控制面板已实现，等待实机回归 |
| 内容/文案 | 设定、脚本、说明文字 | 普通用户安装、权限、手势与排障网页已完成 |
| 美术/资产 | 图片、视觉、素材管理 | 克莱因蓝/薯片黄品牌、正式图标与四张手势线稿已完成 |
| 发布 | GitHub、部署、版本说明 | GitHub Release 与 `chiphand.huyee.art` 均已上线，DMG/ZIP/SHA256 可下载 |

## 当前任务

- 从本地 DMG 按普通用户路径安装，完成一次 Gatekeeper 放行及摄像头/辅助功能授权。
- 实机回归张掌翻页、严格 OK 滚动/导航、食指定位点击、暂停与反馈窗口。
- 验收控制窗口、Dock 图标和内置说明；发现问题先修复并递增版本。
...
- 完成 `0.8.4` 跨线反馈：严格 OK 水平导航确认跨中线时，以黄色捏合球为中心闪现短蓝色发光竖线。
- 完成 `0.8.5` 同步修正：水平导航锁定后第一次到达中线便立即触发，蓝线加长为 `176pt`。
- 完成 `0.8.6` 精简反馈：中线加长为 `352pt` 白线蓝光，关闭骨架后仍保留黄色控制点与跨线闪光。
- 完成 `1.0.0` 品牌与交付：正式命名“薯片手 / ChipHand”，加入完整图标、蓝黄视觉、离线图文说明、MIT/Apache 许可、Universal App、DMG/ZIP 和校验文件。
- 完成 `1.0.1` 控制点修正：删除“显示控制点”开关，清除旧关闭记录，食指指针和黄色控制点永久开启。
- GitHub 仓库：`https://github.com/HuYee2025/chiphand-macos`；旧黑洞仓库仅作为历史远程保留。
