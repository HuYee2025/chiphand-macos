...
- 当前版本：浏览器插件 `1.0.1` 已冻结；macOS 原型 `0.6.0`，`0.3.0` 为 ⭐ 重要稳定回滚版
- 当前目标：验收右手/左手单选控制和原有手势功能
...
| 04 | 进行中 | macOS 系统级手势控制原型 | `0.6.0` 单手选择与严格动作过滤已实现，等待实机验收 |
...
| 开发 | 功能实现、修 Bug、验证 | macOS `0.6.0` 指定右手/左手的严格单手控制 |
...
- 实测默认右手、切换左手、重启后记忆选择，以及另一只手不产生动作。
