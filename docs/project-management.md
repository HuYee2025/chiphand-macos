...
- 当前版本：0.5.0
- 当前目标：用户在 MacBook Chrome 中实机验证捏合上下拖动、左右挥动和真实网站兼容性
...
| 开发 | 功能实现、修 Bug、验证 | 0.5.0 已改为捏合连续滚动、左右挥动翻页，并显示黄色捏合定位球，等待真实手势验收 |
...
- 用户加载 `dist-extension/`，验证真实摄像头、捏合上下拖动和左右挥动。
- 记录 10 次捏合启停成功率、连续滚动稳定性、左右翻页成功率和两分钟误触情况。
