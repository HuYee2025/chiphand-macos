...
- 当前版本：浏览器插件 `1.0.1` 已冻结；macOS 原型 `0.5.1`，`0.3.0` 为 ⭐ 重要稳定回滚版
- 当前目标：验收固定状态条、右侧迷你条、双击暂停/恢复和原有手势功能
...
| 04 | 进行中 | macOS 系统级手势控制原型 | `0.5.1` 窗口交互和暂停状态已实现，等待实机验收 |
...
| 开发 | 功能实现、修 Bug、验证 | macOS `0.5.1` 固定反馈条、右侧迷你条、双击暂停与 Gesture Recognizer |
...
- 实测三道杠收起、迷你条稳定上下移动、双击恢复和双击暂停/恢复。
