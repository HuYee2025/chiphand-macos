...
- 当前版本：浏览器插件 `1.0.1` 已冻结；macOS 原型 `0.7.1`，`0.7.0` 为本轮修改前回滚点，`0.3.0` 为 ⭐ 重要稳定版
- 当前目标：验收方向锁捏合导航、食指跨中线翻页和单手控制
...
| 04 | 进行中 | macOS 系统级手势控制原型 | `0.7.1` 食指翻页已实现，等待真实手势验收 |
...
| 开发 | 功能实现、修 Bug、验证 | macOS `0.7.1` 捏合导航与食指跨中线翻页 |
...
- 实测食指左向右下翻、右向左上翻；确认静止食指和 V 手势不翻页，并回归单手选择、点赞、暂停和反馈窗口。
