...
- 当前版本：浏览器插件 `1.0.1` 已冻结；macOS 原型 `0.7.2`，`0.7.1` 为本轮修改前回滚点，`0.3.0` 为 ⭐ 重要稳定版
- 当前目标：验收严格食指姿态、指尖跨中线翻页和捏合导航
...
| 04 | 进行中 | macOS 系统级手势控制原型 | `0.7.2` 指尖轨迹与严格食指姿态已实现，等待真实手势验收 |
...
| 开发 | 功能实现、修 Bug、验证 | macOS `0.7.2` 捏合导航与严格食指尖翻页 |
...
- 实测握拳竖食指后仅移动指尖即可左右翻页；确认手指散开、静止食指和 V 手势不翻页。
