...
- 当前版本：浏览器插件 `1.0.1` 已冻结；macOS 原型 `0.8.0`，`0.7.3` 为本轮修改前回滚点，`0.3.0` 为 ⭐ 重要稳定版
- 当前目标：验收浏览器食指悬停、张掌点击和黄色指尖定位
...
| 04 | 进行中 | macOS 系统级手势控制原型 | `0.8.0` 食指指针与张掌点击已实现，等待 YouTube 实机验收 |
...
| 开发 | 功能实现、修 Bug、验证 | macOS `0.8.0` 浏览器食指指针与组合点击 |
...
- 验证食指停稳后张掌只点击一次；未停稳张掌不点击、不翻页，直接张掌仍能左右翻页。
