...
- 当前版本：浏览器插件 `1.0.1` 已冻结；macOS `1.1.2 build 25` 已发布，公开 Release 为 `macos-v1.1.2`，`macos-v0.3.0` 为 ⭐ 重要稳定版
...
| 开发 | 功能实现、修 Bug、验证 | `1.1.2` 双手检测、单一菜单栏控制面板和校准窗口直接关闭已实现，等待实机回归 |
| 内容/文案 | 设定、脚本、说明文字 | 普通用户安装、权限、手势与排障网页已完成 |
| 美术/资产 | 图片、视觉、素材管理 | 克莱因蓝/薯片黄品牌、正式图标与四张手势线稿已完成 |
| 发布 | GitHub、部署、版本说明 | `macos-v1.1.2` Release 与 `chiphand.huyee.art` 均已同步，DMG/ZIP/SHA256 可下载 |
