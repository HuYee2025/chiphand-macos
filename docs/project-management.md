...
- 当前版本：浏览器插件 `1.0.1` 已冻结；macOS `1.1.0 build 23` 本地待实机验收，公开 Release 仍为 `macos-v1.0.1`，`macos-v0.3.0` 为 ⭐ 重要稳定版
...
| 开发 | 功能实现、修 Bug、验证 | `1.1.0` 双手检测、控制手优先和标签稳定窗口已实现，等待实机 FPS 与双手回归 |
