...
当前版本状态：
- `0.10.0` Chrome 手势浏览插件以独立控制窗口完成首次摄像头授权和实时预览，再由 Offscreen Document 持续识别；捏合上拖连续下翻，左挥上翻、右挥下翻。捏合点是覆盖网页画面的 28px 真实反色圆，稍微分开即消失；点以 30 FPS 状态同步与动画帧插值绘制。控制窗口关闭时会恢复网页接收器和后台状态，入口在展开态隐藏；第 03 阶段等待用户真实手势验收。
...
- 用户确认点击网页后后台识别仍持续，控制窗口关闭后仍能捏合/挥动控制页面；严格捏合才出现反色点，手指略分开时点应立刻消失。手向上拖能查看下方内容；右挥显示 `↓` 并下滚，左挥显示 `↑` 并上滚。
- 用户确认控制窗口右侧入口不再同时可见，点击网页后窗口关闭、圆点回弹；“关闭摄像头”满宽按钮可用。
- 记录 10 次捏合启停、右挥下翻、左挥上翻成功率，以及两分钟静止误触。
- 未得到发布请求前不要推送 GitHub 或发布 Chrome 商店。

下一步建议：
- 在 `chrome://extensions/` 对此扩展点击重新加载，然后刷新 `http://127.0.0.1:5173/gesture-test.html`；启动摄像头后确认控制窗口底部不再出现 `Receiving end does not exist`。关闭窗口后，拇指食指近乎贴合约 0.1 秒，确认网页 28px 圆内是当前画面的反色，不再恒为白色；略微分开应立即消失。然后点击网页让窗口关闭，仍须能向上捏合拖动、右挥下滚与左挥上滚。展开“高级设置”，分别将两条滑杆调到 30 与 70，确认值保留且立即改变触发门槛；点击小黑圆点确认紧凑预览贴边打开，窗口展开时不应再看到第二个黑点。
