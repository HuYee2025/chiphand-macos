...
- 第 05 阶段：薯片手开源发布（已完成）。
- 当前版本：macOS `1.1.1 build 24`（菜单栏单一控制面板本地待实机验收）；公开稳定版仍为 `macos-v1.0.1`，重要稳定版 `macos-v0.3.0`。
- 当前分支：`main`。
...
- 排障区已从“检查这五件事”更新为“检查这六件事”，新增右手默认控制、单手识别和“请使用右手”反馈说明。
...
- 线上站点验证通过：HTTP 自动跳转 HTTPS，主页、CSS、全部手势图片、DMG、ZIP 和校验文件均返回 `200`，TLS 证书域名正确。
...
- 双手同时入镜时右手是否稳定优先、左手是否完全无效、识别 FPS 是否仍可接受，以及两手重叠时是否会短暂丢手。

## 下一步建议

1. 先安装并实测 `macos-app/build/ChipHand.app` 的 `1.1.1 build 24`，重点验证启动后只出现一个控制面板，并回归双手同时入镜、左手先入镜和两手重叠。
2. 细节优化必须保留食指跟踪流畅度、严格 OK 防误触、单手选择、反馈窗口和权限稳定性。
3. 修改后按风险运行 Swift/Web/Universal 构建与安装验证，并同步更新版本记录。
4. 通过实机验收后再生成 `macos-v1.1.1` Release；不要移动或覆盖 `macos-v1.0.1` 标签。
