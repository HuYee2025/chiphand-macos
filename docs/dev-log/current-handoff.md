...
当前版本状态：
- `0.5.1` Chrome 手势浏览插件以独立控制窗口完成首次摄像头授权和实时预览，再由 Offscreen Document 持续识别；上下控制为拇指食指捏合后的连续拖动，张手仅保留左右翻页。控制窗口离开后直接关闭，网页右侧只保留一个黑色方形动作反馈；第 03 阶段等待用户真实手势验收。
...
- 用户确认点击网页后后台识别仍持续，控制窗口完全关闭且只剩网页内一个方形提示；捏合黄球出现且上下拖动正确，左右挥动正确。
- 记录 10 次捏合启停成功率、连续滚动稳定性、左右翻页成功率和两分钟静止误触。
- 收集拒绝合成左右方向键的网站，再添加站点适配器。
- 未得到发布请求前不要推送 GitHub 或发布 Chrome 商店。

下一步建议：
- 按 README 重新加载 `dist-extension/` 并刷新测试页；启动摄像头后离开控制窗口，确认它完全关闭且网页只留下一个方形提示。再捏住拇指食指约 0.1 秒并上下移动手验证连续滚动；张手后再左右挥动验证翻页。
