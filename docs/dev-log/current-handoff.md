...
- 浏览器插件冻结在本地 `v1.0.1`；macOS 当前版本 `0.8.0`，`macos-v0.7.3` 为本轮修改前回滚点，`macos-v0.3.0` 为 ⭐ 重要稳定版。
...
- 新增默认关闭的食指指针测试：四个浏览器中严格握拳竖食指移动真实鼠标；停稳后张开同一只手执行一次单击，鼠标在退出后停留原位。
- 食指指针使用 150ms 激活、180ms 分类容错、约 24pt/350ms 定位、800ms 张掌转换和 18% 单帧跳变保护；未定位张掌不点击也不翻页。
...
- `swift test`：28 项 XCTest 全部通过，包括食指激活、定位、张掌点击、未定位拦截、分类容错、跳变保护及捏合优先级。
- `swift run GestureControlCoreChecks`：20 项全部通过。
...
- 尚需用户在 YouTube 实测食指鼠标定位、黄色点对齐、悬停预览和张掌单击手感。
...
2. 食指停稳看到待点击提示后张掌，验证只打开一次当前视频；未停稳张掌不得点击或翻页。
3. 验证直接张掌仍能左右翻页，严格 OK 仍能滚动和返回/前进。
