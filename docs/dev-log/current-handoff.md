...
当前版本状态：
- `0.3.2` Chrome 手势浏览插件已改用独立控制窗口，以支持 macOS 摄像头授权；摄像头启动后 5 秒会缩为屏幕右侧纯黑标签，悬停自动展开；第 03 阶段等待用户真实手势验收。
...
- 新增控制窗口自动紧凑模式：启动后 5 秒缩成 92×142 的右侧标签；悬停自动展开，离开约半秒或停留 5 秒自动收回，摄像头不中断。
- 紧凑标签平时纯黑；识别到动作时，短暂显示对应的绿色大箭头。
...
- `0.3.2` 悬停展开、离开/5 秒收起和方向箭头反馈通过 TypeScript、20 项手势测试及完整构建；真实 Chrome popup 的悬停体验仍需用户 MacBook 验收。

当前未完成：
- 用户在目标 MacBook 上重新加载 `dist-extension/`，从测试页点击插件图标打开控制窗口，再用真实手掌测试四个方向。
- 用户确认紧凑标签的悬停展开/收起和主显示器或外接屏位置符合预期；如需要真正贴边悬停自动探出且不占 Chrome 窗口层级，后续改为 macOS 原生菜单栏应用。
- 每方向 10 次成功率、重复触发次数、两分钟静止误触需要真实手势记录。
- 收集拒绝合成左右方向键的网站，再添加站点适配器。
- 未得到发布请求前不要推送 GitHub 或发布 Chrome 商店。

下一步建议：
- 按 README 重新加载 `dist-extension/`，启动摄像头并等待 5 秒，确认控制窗口缩为右侧纯黑标签；悬停、移开并挥动四个方向后，在 `http://127.0.0.1:5173/gesture-test.html` 验收。
