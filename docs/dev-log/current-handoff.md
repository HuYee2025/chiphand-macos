...
- 浏览器插件冻结在本地 `v1.0.1`；macOS 当前版本 `0.7.0`，`macos-v0.6.0` 为本轮修改前回滚点，`macos-v0.3.0` 为 ⭐ 重要稳定版。
...
- V 手势改为左右跨中线翻页：左向右下翻、右向左上翻；张开手掌只显示状态，不再执行动作。
...
- `swift test`：18 项 XCTest 全部通过，包括捏合方向锁、返回/前进、V 双向翻页、张手空操作、严格 OK 和控制手过滤。
- `swift run GestureControlCoreChecks`：12 项全部通过。
...
- 尚需用户用真实手势验收四个浏览器的严格 OK 返回/前进、V 双向翻页和垂直滚动方向锁。
