...
- 浏览器插件冻结在本地 `v1.0.1`；macOS 当前版本 `0.7.1`，`macos-v0.7.0` 为本轮修改前回滚点，`macos-v0.3.0` 为 ⭐ 重要稳定版。
...
- 食指手势改为左右跨中线翻页：左向右下翻、右向左上翻；V 手势和张开手掌只显示状态，不执行动作。
...
- `swift test`：19 项 XCTest 全部通过，包括捏合方向锁、返回/前进、食指双向翻页、V/张手空操作、严格 OK 和控制手过滤。
- `swift run GestureControlCoreChecks`：13 项全部通过。
...
- 尚需用户用真实手势验收四个浏览器的严格 OK 返回/前进、食指双向翻页和垂直滚动方向锁。
...
3. 验证食指左向右下翻、右向左上翻；静止食指、V 和张开手掌均不翻页。
