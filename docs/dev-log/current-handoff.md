...
当前版本状态：
- `1.0.0` Chrome / Microsoft Edge（Chromium）手势浏览插件已完成可交付构建：独立控制窗口完成首次摄像头授权和实时预览，再由 Offscreen Document 持续识别；捏合上拖连续下翻，左挥上翻、右挥下翻。网页默认不显示捏合点或手部网格；“高级设置”始终显示在启停按钮下方，可按需开启两项调试叠层。窗口只在鼠标移出范围后收起，失焦不会关闭；控制窗口关闭时会恢复网页接收器和后台状态，入口在展开态隐藏。
...
下一步建议：
- 在 `chrome://extensions/` 或 `edge://extensions/` 对此扩展点击重新加载，然后刷新 `http://127.0.0.1:4173/gesture-test.html`；确认版本显示为 `1.0.0`。不启动摄像头时，“高级设置”已在启停按钮下方；展开后应出现两条滑杆和两个开发者开关。启动后鼠标留在窗口内，窗口不得消失；移出窗口后才收起。关闭窗口后，仍须能向上捏合拖动、右挥下滚与左挥上滚。点击小黑圆点确认紧凑预览贴边打开，窗口展开时不应再看到第二个黑点。
- 商店发布时，使用 `releases/gesture-browser-control-v1.0.0.zip`；本机安装仍选择 `dist-extension/`，不要选择 ZIP。
