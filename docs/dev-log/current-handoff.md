...
- 浏览器插件冻结在本地 `v1.0.1`；macOS 当前版本 `0.8.1`，`macos-v0.8.0` 为本轮修改前回滚点，`macos-v0.3.0` 为 ⭐ 重要稳定版。
...
- 新增默认关闭的食指指针测试：四个浏览器中严格握拳竖食指移动真实鼠标；停稳后保持食指并张开拇指执行一次单击，鼠标在退出后停留原位。
- 食指指针使用 150ms 激活、180ms 分类容错、约 24pt/350ms 定位、120ms 拇指张开确认和 18% 单帧跳变保护；拇指收回 150ms 后才能重新点击。
- 食指资格与拇指状态已拆分：拇指展开过程允许 350ms 中间态，黄色指尖点按食指和其余三指的几何姿态持续显示。
...
- 尚需用户在 YouTube 实测食指鼠标定位、黄色点对齐、张开拇指单击手感，以及链接是否始终在当前页打开。
...
2. 食指停稳看到待点击提示后张开拇指，验证只打开一次当前视频且始终使用当前页；拇指保持张开不得连点。
