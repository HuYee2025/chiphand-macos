...
- 浏览器插件冻结在本地 `v1.0.1`；macOS 当前版本 `0.7.3`，`macos-v0.7.2` 为本轮修改前回滚点，`macos-v0.3.0` 为 ⭐ 重要稳定版。
...
- `swift test`：20 项 XCTest 全部通过，包括张开手掌无需跨中线双向翻页、冷却/重激活、捏合导航及食指/V 留空。
- `swift run GestureControlCoreChecks`：14 项全部通过。
...
3. 验证张开手掌右挥下翻、左挥上翻；无需跨中线，静止手掌、食指和 V 均不翻页。
