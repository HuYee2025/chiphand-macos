...
- `PinchScrollDetector`：拇指食指接触稳定 80ms 后接管垂直控制；接触/释放阈值采用迟滞，连续输出受跳变保护的归一化滚动量。
- `SwipeDetector`：保存最近 360ms 手掌中心轨迹；Extension 仅启用左右方向翻页，保留 650ms 冷却和稳定/离手重新激活。
- Extension 控制窗口：负责首次请求摄像头权限、实时预览、显示启动状态和停止控制；后台识别已运行时重新打开窗口会自动恢复预览。鼠标离开后窗口直接关闭，失焦或关闭不停止后台识别；需要停止时从插件图标重新打开。
...
- 网页内状态提示：通过 Content Script 固定在受控网页右侧；始终为 88×88 的黑色正方形，识别动作时短暂显示对应绿色大箭头。
