...
- `AppModel`：权限、摄像头、手势状态和前台 App 目标的唯一协调者；持久保存右手/左手单选，只有所选手能进入 `GestureEngine`，App 切换或控制手切换立即取消活动手势。SwiftUI 只保留 `MenuBarExtra` 控制面板，不创建重复的常驻 `Window`；校准预览窗口可通过标题栏关闭并同步设置。
