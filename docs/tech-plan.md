...
- `GestureControlCore`：平台无关的关键点归一化、严格 OK 捏合迟滞与方向锁、跨中线导航、张掌翻页，以及严格食指指针/停稳/拇指张开点击组合状态机；V 手势仅保留状态。
...
- `SystemPointerEmitter`：把镜像 `indexTip` 直接映射到主屏幕并发送 HID `mouseMoved`；定位后张开拇指，在冻结点使用独立 `.privateState` 事件源发送一次无修饰键左键按下/松开。只在四个浏览器和辅助功能有效时启用。
