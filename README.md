...
授权完成后，控制窗口保留实时摄像头预览，识别转入 Chrome 后台页面：你可以直接点击和浏览网页，识别不会中断。Chrome 只会在首次授权时弹窗；此前已经允许时会直接启动。当前网页右侧会出现一个极细的黑色提示条；它不会因点击网页消失。鼠标悬停后会展开为状态提示，鼠标移出后才会收回。只有识别到挥动时，提示条才会短暂显示对应的绿色大箭头。
