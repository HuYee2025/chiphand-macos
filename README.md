...
授权完成后，控制窗口保留实时摄像头预览，识别转入 Chrome 后台页面：你可以直接点击和浏览网页，识别不会中断。鼠标离开控制窗口后它会直接关闭，不会留下第二个黑色标签；需要停止摄像头时重新点击插件图标。Chrome 只会在首次授权时弹窗；此前已经允许时会直接启动。拇指与食指捏住约 0.1 秒后，上下移动手即可连续滚动网页（手向上，页面向上；松开即停）；张开手掌向左/右挥动用于翻页。捏合时预览会在两指之间显示黄色定位球。当前网页右侧会一直保留一个 88×88 的黑色正方形提示；点击它可重新打开摄像头预览，识别到动作时会短暂显示对应的绿色大箭头。
