...
- 严格 OK 水平锁定后首次到达中线时：以黄色捏合球为中心同步闪现一条 `352pt` 白色核心、蓝色外发光竖线，表示已可松手。
- 张开手掌向右挥动：向下翻约 75% 屏幕；向左挥动：向上翻约 75% 屏幕。达到水平挥动距离即可，不要求跨过屏幕中线。
- 可选“显示控制点”：在 Chrome、Safari、Edge、夸克中握拳竖食指移动真实鼠标；指尖停稳后直接让拇指与中指轻捏接触，执行一次普通左键点击。默认关闭。
- V 手势：只显示识别状态，暂不执行操作。
- 竖起拇指：显示点赞识别反馈，不执行真实点赞。
- 除主动开启“显示控制点”外，不移动鼠标光标或发送鼠标点击；指针仅支持 MacBook 主屏幕和四个浏览器。
